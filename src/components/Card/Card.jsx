...
function Card({ user, loading }) {
  const joiningDate = new Date(user.created_at).getDate();
  const joiningMonth = new Date(user.created_at).toLocaleString("default", {
    month: "short",
  });
  const joiningYear = new Date(user.created_at).getFullYear();
  const userNotFoundHTML = (
    <div className="user-not-found">
      <figure className="user-image__wrapper">
        <img
          src={notFoundImage}
  ...
  return (
    <div className={user.message || loading ? "card card-not-found" : "card"}>
      {loading ? (
        <img src={loader} alt="user info loading" className="loader" />
      ) : user.message ? (
        userNotFoundHTML
      ) : (
        <div className="user">
          ...
          <div className="user-info">
            <div className="user-info__block">
              <div className="user__main-info">
                <h2 className="user-realname">{user.name}</h2>
                <a href={user.html_url} className="username">
                  @{user.login}
                </a>
              </div>
              <p className="user__joining-date">
                Joined {joiningDate} {joiningMonth} {joiningYear}
              </p>
            </div>
            <div className="user-description">{user.bio}</div>
            <div className="user-data">
              <div className="user-data__block">
                <p className="user-block__subheading">Repos</p>
                <h4 className="user-block__heading">{user.public_repos}</h4>
              </div>
              <div className="user-data__block">
                <p className="user-block__subheading">Followers</p>
                <h4 className="user-block__heading">{user.followers}</h4>
              </div>
              <div className="user-data__block">
                <p className="user-block__subheading">Following</p>
                <h4 className="user-block__heading">{user.following}</h4>
              </div>
            </div>
            <ul className="user-links__list">
              <li
                className={
                  user.location === null
                    ? "user-links__list-item link-disabled"
                    : "user-links__list-item"
                }
              >
                <svg
                  height="20"
              ...
              >
                ...
                {user.twitter_username === null ? (
                  ...
                  <a
                    ...
                    target="_blank"
                    className="user-links__list-item-link"
                    rel="noreferrer"
                  >
